...
namespace AspComet
{
    public class Client : IClient
    {
        ...
        private void HandleTimerCallback(object state, ElapsedEventArgs e)
        {
            ...
            if ((now - this.LastMessageTime).TotalMilliseconds > AspComet.Middleware.CometMiddleware.LongPollDurationInMilliseconds)
            {
                this.FlushQueue();
            }
            if ((now - this.LastConnectTime).TotalMilliseconds > AspComet.Middleware.CometMiddleware.ClientTimeoutInMilliseconds)
            {
                this.Disconnect();
            }
        }
